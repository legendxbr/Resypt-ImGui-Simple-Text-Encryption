...
namespace window
{
    ...
    bool create_window( std::string windowname, std::pair<int, int> size)
    {
		...
		long windowflag = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX;
        vars::hwnd = LI_FN( CreateWindowExA ).cached( )( 0, vars::wc.lpszClassName, windowname.c_str( ), windowflag, 100, 100, size.first, size.second, nullptr, nullptr, vars::wc.hInstance, nullptr );
